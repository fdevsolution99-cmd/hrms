  ...
  const fetchLeaves = async () => {
    try {
      ...
      if (error.response && !error.response.data.success) {
        ...
        alert("An error occurred while fetching leaves. Check console for details.");
      }
      setFilteredLeaves([]); // Set to empty to clear the "Loading ..." state
    }
  };
  useEffect(() => {
    fetchLeaves();
              ...
              <div key={leave._id} className="bg-white rounded-lg shadow-md p-4 mb-4 border border-gray-200">
                <div className="flex justify-between items-start mb-3">
                  <div className="flex items-center gap-2">
                    ...
                    <span className={`px-2 py-1 rounded text-xs font-semibold ${leave.status === "Approved"
